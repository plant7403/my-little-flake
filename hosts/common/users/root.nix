{...}: {
  #sops.secrets."users/root/password" = {};
  users.mutableUsers = false;
  users.users.root = {
    #isNormalUser = true;
    #shell = pkgs.zsh;
    #openssh.authorizedKeys.keys = [ (builtins.readFile ../../../../home/egor/ssh.pub) ];
    # FIXME - Move to sops
    openssh.authorizedKeys.keys = [
      "ssh-rsa  cardno:19_271_673"
    ];
    #passwordFile = config.sops.secrets.egor-password.path;
    #    password = config.sops.secrets."users/root".path;

    #packages = [ pkgs.home-manager ];
  };
}
# mkdir /root/.ssh/ && echo "ssh-rsa  cardno:19_271_673" > /root/.ssh/authorized_keys

